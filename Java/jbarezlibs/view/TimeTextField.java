...
/**
 * Textfield that forces time introduction Style: 00:00:00. Thanks to:
 * http://stackoverflow.com/questions/17188712/text-field-validation-in-javafx
 *
...
public class TimeTextField extends TextField {
    ...
    public TimeTextField(String time) {
        ...
        totalSeconds = new ReadOnlyIntegerWrapper(this, "totalSeconds");
        ChangeListener<Number> totalSecondsUpdater = new ChangeListener<Number>() {
            @Override
            public void changed(ObservableValue<? extends Number> ov, Number t, Number t1) {
                actualizarTotalSeconds();
            }
        };
        seconds.addListener(totalSecondsUpdater);
        minutes.addListener(totalSecondsUpdater);
        hours.addListener(totalSecondsUpdater);
    }

    private void actualizarTotalSeconds() {
        totalSeconds.setValue((getHours() * 3600) + (getMinutes() * 60) + getSeconds());
    }
    ...
    private boolean validate(String time) {
        ...
        try {
            int hours_ = Integer.parseInt(tokens[0]);
            int mins_ = Integer.parseInt(tokens[1]);
            int secs_ = Integer.parseInt(tokens[2]);
